            ...
            <div>
                ...
                <div className="flex w-full bg-white py-1 px-4 items-center justify-center rounded-full mb-6">
                    <h3 className='text-base font-bold text-[#1A234E]'>
                       Nº Apólice: {produto.numeroApolice} 
                    </h3>
                </div>

                {/* 3. Texto branco e com mais espaço entre as linhas (space-y-2) */}
                <div className='px-2 mb-8 text-white space-y-2 font-medium'>
                    <p>Ano do carro: {produto.anoCarro}</p>
                    <p>Valor: {produto.valor}</p>
                    <p>Data inicio: {new Intl.DateTimeFormat("pt-BR", {
                        dateStyle: 'full',
                        timeStyle: 'medium',
                    }).format(new Date(produto.dataInicio))}</p>
                    <p>Data fim: {new Intl.DateTimeFormat("pt-BR", {
                        dateStyle: 'full',
                        timeStyle: 'medium',
                    }).format(new Date(produto.dataFim))}</p>
                    
                    {/* Cores diferentes para Usuario e Categoria para destacar */}
                    {/* <p className="text-blue-300 pt-2">Usuario: </p> */}
                    <p className="text-blue-300">Categoria: {produto.categoria?.descricao} </p>
                </div>
            </div>
